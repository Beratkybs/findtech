...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<String>> handleBaseException(BaseException e, WebRequest request) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(createApiError(e.getMessage(), request));
    }


    public <T> ApiResponse<T> createApiError(T message, WebRequest request) {
